...
import DrugView from './opd_drug-view';

function DrugTable({ columns, getHeaderProps, handleAdd }) {
  ...
  console.log("drugs initial",drugs);
  ...
  useEffect(() => {
    ...
    dispatch(getDrugs());
    console.log("drugs after dispatch",drugs);
  ...
  const renderRowSubComponent = useCallback(
    ({ row }) => {
      return <DrugView data={drugs.find((drug) => drug._id === row.values._id)} />;
    },
    [drugs]
  );
      ...
      <Stack spacing={3}>
        ...
        >
          ...
          <Stack direction={matchDownSM ? 'column' : 'row'} alignItems="center" spacing={1}>
            ...
            <CSVExport data={selectedFlatRows.length > 0 ? selectedFlatRows.map((d) => d.original) : drugs} filename={'drug-list.csv'} />
          </Stack>
        </Stack>
